...
router.get("/test", (req, res) => res.json({ msg: "Posts wors" }));
//@route GET api/posts
...
router.get("/", (req, res) => {
  Post.find()
    ...
    .catch(err =>
      res.status(404).json({ noposts: "There are currently no posts" })
    );
});
...
router.post(
  ...
  (req, res) => {
    //Pass req.body to validate Post Input
    const { errors, isValid } = validatePostInput(req.body);
    //Check the validation
    if (!isValid) {
      // If theres any errors, send 400 with errors
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      Post.findById(req.params.id).then(post => {
        ...
        if (post.user.toString() !== req.user.id) {
          return res
            .status(401)
            .json({ notAuthorized: "You are not authorized to delete posts" });
        }
        //Delete
        post
          .remove()
    ...
    Profile.findOne({ user: req.user.id }).then(profile => {
      Post.findById(req.params.id)
        .then(post => {
          ...
          //Get remove index
          let removeIndex = post.likes.map(item =>
